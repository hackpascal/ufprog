			...
			size_t *period)
{
  ...
  */
  ...
  while (j + k < needle_len)
    {
      a = (unsigned char) CANON_ELEMENT (needle[j + k]);
      b = (unsigned char) CANON_ELEMENT (needle[(size_t)(max_suffix + k)]);
      if (a < b)
	{
  ...
  *period = p;
  ...
  while (j + k < needle_len)
    {
      a = (unsigned char) CANON_ELEMENT (needle[j + k]);
      b = (unsigned char) CANON_ELEMENT (needle[max_suffix_rev + k]);
      if (b < a)
	{
	  /* Suffix is smaller, period is entire prefix so far.  */
	  j += k;
		      ...
		      const unsigned char *needle, size_t needle_len)
{
  ...
  /* Perform the search.  Each iteration compares the right half
     first.  */
  if (CMP_FUNC ((const char *)needle, (const char *)needle + period, suffix) == 0)
    {
